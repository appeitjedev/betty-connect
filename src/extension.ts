...
export function activate(context: vscode.ExtensionContext) {
  ...
  if (vscode.workspace.workspaceFolders) {
    var bettyapp = String(
      vscode.workspace.getConfiguration().get("betty.bettyapp")
    );
    var username = String(
      vscode.workspace.getConfiguration().get("betty.username")
    );
    var apiKey = String(
      vscode.workspace.getConfiguration().get("betty.apikey")
    );
    function loadTemplates() {
      if (apiKey && username && bettyapp) {
        ...
        vscode.window.showErrorMessage(
          "Connection failed!! try betty connect or betty reload templates"
        );
      }
    }
    ...
    vscode.commands.registerCommand("extension.connect", async () => {
      ...
      apiKey = String(
        await vscode.window.showInputBox({
          ...
          ignoreFocusOut: true,
          prompt:
            "Type in your api key for the user account from the previous input. You can get this if you can get the the user management in my.bettyblocks.com",
        })
      );
      if (apiKey !== "") {
        await vscode.workspace
